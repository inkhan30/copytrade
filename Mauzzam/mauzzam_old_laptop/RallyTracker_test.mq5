...
#property version   "1.01"               // Updated with breakeven buffer
...
input double   StepPips          = 300.0;            // Price step between levels (pips)
input double   InitialStopPips    = 300.0;           // Initial stop loss for first position (pips)
input double   BreakevenBufferPips = 100.0;          // Buffer after each new level (pips)
input double   TrailingStopPips   = 50.0;            // Trailing stop after all levels opened (pips)
   ...
   if(!trailingActive && currentLevel < MaxLevels)
   {
      ...
      if(trigger)
      {
         ...
         if(trade.ResultRetcode() == TRADE_RETCODE_DONE)
         {
            ...
            // --- NEW: Set stop loss with a buffer from the latest entry ---
            if(direction == 1)
               stopLossPrice = lastEntryPrice - BreakevenBufferPips * pipMultiplier;
            else
               stopLossPrice = lastEntryPrice + BreakevenBufferPips * pipMultiplier;

            stopLossPrice = NormalizePrice(stopLossPrice);

            //--- move all positions' stop loss to the new price
