//+------------------------------------------------------------------+
//| Expert Advisor: Dynamic Hedging RallyTrackerPro_v10_3_1           |
//+------------------------------------------------------------------+
#property copyright "Copyright 2026, RallyTrackerPro_v10_3_1"
#property link      "https://www.mql5.com"
#property version   "10.03"
#property strict
#property indicator_chart_window

// Hard-coded array of allowed account numbers
const long ALLOWED_ACCOUNT_NUMBERS[] = {11588786}; // Add your multiple account numbers here eg. 123456,8765434

// --- EXPIRY DATE (MODIFY THIS LINE TO SET/UPDATE EXPIRY) ---
const datetime EXPIRY_DATE = D'2026.12.31';  // EA will stop working after this date
...
// New RSI Filter Inputs
input bool     UseRSIFilter        = false;      // Enable/disable RSI based entry filter
...
input int      RSIDirection        = 0;          // 0=Both sides, 1=Oversold only, 2=Overbought only
...
int OnInit()
{
   ...
   // Check account number restriction
   if(!IsAccountAllowed())
   {
   ...
   if(now >= EXPIRY_DATE)
   {
      Print("EXPIRY DATE REACHED. EA will not start. Expiry: ", TimeToString(EXPIRY_DATE), ", Current: ", TimeToString(now));
      strategyEnabled = false;
      CreateInfoLabels();          // Still create labels to show status
      UpdateStatusLabel("Expired. Contact Developer for Renewal");
      return INIT_FAILED;       // Allow EA to load but remain inactive
   }
   // -------------------------
   ...
   // Validate RSIDirection input
   //if(RSIDirection < 0 || RSIDirection > 2)
   //{
      //Print("Error: RSIDirection must be 0, 1, or 2! Using default (0)");
      //RSIDirection = 0;
   //}
...
double GetRSIValue()
{
   if(rsiHandle == INVALID_HANDLE) return 50; // Return neutral value if RSI not available
...
bool CheckRSIFilter(int &dir)
{
   ...
   if(rsiValue == 50) return false; // Failed to get RSI value
   
   // Check RSI direction based on the new parameter
   if(RSIDirection == 1) // Oversold only
   {
      if(rsiValue <= RSI_Oversold)
      {
         dir = 1; // Only allow buy trades (oversold condition)
         Print("RSI Filter: Oversold (", DoubleToString(rsiValue, 2), ") - Buy only (Oversold mode)");
         return true;
      }
      else
      {
         Print("RSI Filter: RSI not in oversold zone (", DoubleToString(rsiValue, 2), ") - No trades");
         return false;
      }
   }
   else if(RSIDirection == 2) // Overbought only
   {
      if(rsiValue >= RSI_Overbought)
      {
         dir = -1; // Only allow sell trades (overbought condition)
         Print("RSI Filter: Overbought (", DoubleToString(rsiValue, 2), ") - Sell only (Overbought mode)");
         return true;
      }
      else
      {
         Print("RSI Filter: RSI not in overbought zone (", DoubleToString(rsiValue, 2), ") - No trades");
         return false;
      }
   }
   else // RSIDirection = 0 (Both sides)
   {
      if(rsiValue <= RSI_Oversold)
      {
         dir = 1; // Allow buy trades (oversold condition)
         Print("RSI Filter: Oversold (", DoubleToString(rsiValue, 2), ") - Buy only");
         return true;
      }
      else if(rsiValue >= RSI_Overbought)
      {
         dir = -1; // Allow sell trades (overbought condition)
         Print("RSI Filter: Overbought (", DoubleToString(rsiValue, 2), ") - Sell only");
         return true;
      }
      
      Print("RSI Filter: Neutral (", DoubleToString(rsiValue, 2), ") - No trades allowed");
      return false; // RSI in neutral zone - no trades
   }
}
...
void PrintConfiguredLotSizes()
{
   ...
   // Print RSI direction mode
   if(UseRSIFilter)
   {
      string rsiModeStr = "";
      switch(RSIDirection)
      {
         case 0: rsiModeStr = "Both sides (oversold & overbought)"; break;
         case 1: rsiModeStr = "Oversold only (buy trades)"; break;
         case 2: rsiModeStr = "Overbought only (sell trades)"; break;
      }
      Print("RSI Filter Mode: ", rsiModeStr);
   }
}
...
void OnTick()
{
   // Check account number restriction on every tick
   if(!IsAccountAllowed())
   {
      if(strategyEnabled)
      {
         ...
         CloseAllTrades();
         UpdateStatusLabel("Wrong Account");
         Alert("EA stopped - wrong account number!");
      }
      return;
   }
   ...
   if(now >= EXPIRY_DATE)
   {
      if(strategyEnabled)          // If still active, close everything and disable
      {
         CloseAllTrades();
         strategyEnabled = false;
         UpdateStatusLabel("Expired. Contact Developer for Renewal");
         Print("EXPIRY DATE REACHED. All positions closed. EA stopped.");
      }
   ...
   if(equityStopTriggered && RestartAfterDrawdown)
   {
      ...
      Print("Equity Check: Current",currentEquity," :: recoveryLevel",recoveryLevel);
   ...
   if(totalTrades == 0)
   {
      // Check RSI filter first if enabled (when RSI is on, candle-based entry is ignored)
      int rsiDirection = 0;
      ...
      if(UseRSIFilter)
      {
         ...
         if(!rsiConditionMet)
         {
            return; // RSI filter condition not met
         }
      }
      
      // If RSI filter is not enabled, use EMA filter and consecutive candles
      if(!UseRSIFilter)
      {
         ...
         {
            return; // Consecutive candles condition not met
         }
      }
      else
      {
         // RSI filter is enabled and condition is met
         direction = rsiDirection;
      }
      
      // If we get here, either RSI condition is met OR (EMA + consecutive candles) condition is met
   ...
   //Print("Total Profit:",totalProfit," ::: ",ProfitTargetPips * PIP * _Point);
