   ...
   //| (with ATR display + ATR-based stop)                             |
   //+------------------------------------------------------------------+
   ...
   const long ALLOWED_ACCOUNT_NUMBERS[] = {26630694};
   ...
   input int      LastPositionSLPips  = 200;
   ...
   string atrLabel = "ATRLabel";      // ATR label
   string atrValue = "ATRValue";      // ATR value
   ...
   int OnInit()
   {
      ...
      atrStopTriggered = false; // Initialize ATR stop flag
   ...
   void OnTick()
   {
      ...
      // If ATR stop is active, skip all trading logic
      if(atrStopTriggered)
      {
         // Still update chart info and ATR display
         UpdateChartInfo();
   ...
   void UpdateStatusLabel(string text)
   {
      ...
      else if(StringFind(text, "High ATR") >= 0) clr = clrOrange;   // NEW: color for high ATR
   ...
   void ManageHedging()
   {
      ...
      if(direction == 1 && currentPrice <= triggerPrice) conditionMet = true;
      else if(direction == -1 && currentPrice >= triggerPrice) conditionMet = true;
      if(conditionMet)
      {
   ...
   void RemoveInitialPositionTP()
   {
      ...
      for(int i = PositionsTotal()-1; i >=0; i--)
      {
         ...
         if(posTime < earliestTime) { earliestTime = posTime; initialTicket = ticket; }
      }
   ...
   double CalculateHedgeTriggerPrice(int positionCount)
   {
      ...
      int totalPips = 0;
      for(int i = 0; i < positionCount; i++) totalPips += triggerPips.At(i);
      if(direction == 1)
